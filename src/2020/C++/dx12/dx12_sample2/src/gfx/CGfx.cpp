...
bool CGfx::Initialize(HWND hWnd, int width, int height)
{
    m_ViewPort = { 0, 0, static_cast<float>(width), static_cast<float>(height), D3D12_MIN_DEPTH, D3D12_MAX_DEPTH };
    ...
    m_SwapChain = CreateSwapChain(factory.Get(), m_CommandQueue.Get(), FRAME_COUNT, width, height, hWnd);
...
void CGfx::WaitForPreviousFrame()
{
